...
class MainSecureStorage {
  ...
  /// check if instructions is seen ====>>>
  Future<bool> isInstructionsFinished() async {
    final String? instructionsFinish = await _secureStorage.read(
      key: AppConst.instructionsFinishBox,
      aOptions: _getAndroidOptions(),
      iOptions: iosOptions,
    );
    return instructionsFinish == 'true';
  }
  ...
  /// login ====>>>
  Future<void> login(LoginResponseModel login) async {
    AppConst.isLogin = true;
    await Future.wait([
      saveToken(login.token),
      saveUserId(login.user.id),
      // putIsVerified(login.user.isProfileCompleted),
