...
extension ConvertDinarToIQD on String {
  ...
  bool availableForPayment() {
    return this == OrderStatusEnum.waiting_advance_payment.name ||
        this == OrderStatusEnum.waiting_final_payment.name;
  }

  bool availableForCancel() {
    return this != OrderStatusEnum.vendor_rejected.name &&
        this != OrderStatusEnum.completed_by_vendor.name &&
        this != OrderStatusEnum.finished.name &&
        this != OrderStatusEnum.waiting_final_payment.name &&
        this != OrderStatusEnum.cancelled.name &&
        this != OrderStatusEnum.in_progress.name;
  }
  ...
  /// convert string date to 09:00 PM | الثلاثاء 2 سبتمبر
  String get convertDateTimeSeparated {
    try {
      final DateTime dateTime = DateTime.parse(this).toLocal();
  ...
  /// convert 12:54:00 to 12:54 PM
  String get convertTimeTo12HourFormat {
    try {
      final DateTime dateTime = DateFormat('HH:mm:ss', 'en_US').parse(this);
      return DateFormat(
        'hh:mm a',
        NavigatorKey.context.locale.languageCode,
      ).format(dateTime).toUpperCase().toEnglishNumbers;
    } catch (e) {
      return this;
    }
  }
